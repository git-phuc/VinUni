...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
            assistant_reply = assistant_reply + delta
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    def short_text(text: str, max_length: int = 40) -> str:
        text = str(text)
    ...
    for result in results:
        prompt = short_text(result.get("prompt", ""))
        gpt4o_response = short_text(result.get("gpt4o_response", ""))
        mini_response = short_text(result.get("mini_response", ""))
        gpt4o_latency = result.get("gpt4o_latency", 0)
        mini_latency = result.get("mini_latency", 0)

        line = (
            f"{prompt} | "
            f"{gpt4o_response} | "
            f"{mini_response} | "
            f"{gpt4o_latency:.3f}s | "
            f"{mini_latency:.3f}s"
        )
        lines.append(line)
...
if __name__ == "__main__":
    test_prompt = "Hay gioi thieu Viet Nam trong 2 cau ngan."
    result = compare_models(test_prompt)

    print("=== GPT-4o ===")
    print(result["gpt4o_response"])
    print(f"Latency: {result['gpt4o_latency']:.3f}s")
    print(f"Cost estimate: ${result['gpt4o_cost_estimate']:.6f}")

    print("\n=== GPT-4o-mini ===")
    print(result["mini_response"])
    print(f"Latency: {result['mini_latency']:.3f}s")
