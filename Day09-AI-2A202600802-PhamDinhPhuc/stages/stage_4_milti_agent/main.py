...
def search_privacy_law(query: str) -> str:
    """Search privacy and data protection knowledge base for GDPR and privacy issues.

    Args:
        query: Natural language query about data privacy or GDPR.
    """
    knowledge = [
        (
            ["data", "privacy", "gdpr", "consent", "dữ liệu"],
            "GDPR requires a lawful basis for processing personal data, clear notice, "
            "data minimization, and respect for data-subject rights. Serious violations "
            "can trigger fines up to EUR 20M or 4% of global annual turnover.",
        ),
        (
            ["ccpa", "sale", "sharing", "consumer", "user"],
            "CCPA/CPRA requires notice, opt-out rights for sale or sharing of personal "
            "information, and reasonable security. Intentional violations can reach "
            "$7,500 per violation after notice and cure rules where applicable.",
        ),
        (
            ["breach", "security", "incident", "leak"],
            "Privacy incidents may require regulator and affected-user notification. "
            "Failure to notify can increase regulatory penalties, class action exposure, "
            "and FTC unfair/deceptive practice claims.",
        ),
    ]
    query_lower = query.lower()
    results = []
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        ...
        "data-subject rights, breach notification, and cross-border data transfer. "
        "Use the search_privacy_law tool to ground your analysis. Keep your response "
        "under 200 words."
    )
...
QUESTION = (
    "If a tech company breaks a contract, avoids taxes, and shares user data "
    "without consent, what are the legal and regulatory consequences?"
)
...
async def main():
    ...
    graph_png_path = Path(__file__).with_name("stage_4_graph.png")
    graph_png_path.write_bytes(graph.get_graph().draw_mermaid_png())
    print(f"\n[Graph image saved] {graph_png_path}")
