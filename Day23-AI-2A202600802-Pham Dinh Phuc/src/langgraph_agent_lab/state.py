...
class AgentState(TypedDict, total=False):
    ...
    # Student-added fields. All four are OVERWRITE (latest value wins), not append-only:
    # each represents the *current* decision/value for the run, not an audit trail —
    # the audit trail lives in `events`/`messages` (append-only) below.
    evaluation_result: str  # retry-loop gate: "success" | "needs_retry" (read by route_after_evaluate)
    pending_question: str  # clarification flow: question asked when info is missing
    proposed_action: str  # risky-action flow: description of the side-effecting action awaiting approval
    approval: dict[str, Any] | None  # HITL decision: {"approved": bool, "reviewer": str, "comment": str}
