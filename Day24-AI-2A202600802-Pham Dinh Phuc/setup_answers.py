...
def build_pipeline():
    ...
    # Skip enrichment to avoid Gemini API quota timeouts; use raw chunks for indexing.
    print(f"  ✓ Using {len(all_chunks)} raw chunks (enrichment skipped)")
...
def run_query(q: str, search, reranker, top_k: int) -> tuple[str, list[str]]:
    ...
    ctx = "\n\n".join(contexts)
    system_msg = "Trả lời CHỈ dựa trên context. Nếu không có → nói 'Không tìm thấy.'"
    user_msg = f"Context:\n{ctx}\n\nCâu hỏi: {q}"

    # Try Gemini first (with hard timeout to avoid 429 retry waits)
    if GEMINI_API_KEY:
        ...
        try:
            ...
            _client = _genai.Client(
                ...
                http_options=_gtypes.HttpOptions(timeout=15000),
            )
            with _cf.ThreadPoolExecutor(max_workers=1) as _ex:
                _fut = _ex.submit(_client.models.generate_content,
                                  model="gemini-2.5-flash",
                                  contents=f"{system_msg}\n\n{user_msg}")
                resp = _fut.result(timeout=20)
            return resp.text, contexts
        except Exception as e:
            print(f"  ⚠️  Gemini generation failed: {type(e).__name__}")
