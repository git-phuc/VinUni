...
def _call_llm(system_prompt: str, user_content: str, max_tokens: int = 300) -> str:
    """Helper: gọi LLM (Gemini ưu tiên, sau đó 9router, cuối OpenAI)."""
    # Try Gemini (using google.genai SDK)
    if GEMINI_API_KEY:
        try:
    ...
    if NINEROUTER_API_KEY:
        try:
            ...
            client = OpenAI(api_key=NINEROUTER_API_KEY, base_url=NINEROUTER_BASE_URL)
            ...
            print(f"  ⚠️  9router call failed: {e}")
...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    if result:
        ...
        return [q.strip().lstrip("0123456789.-) ").strip() for q in questions if q.strip()][:n_questions]
...
def extract_metadata(text: str) -> dict:
    ...
    """
    result = _call_llm(
        system_prompt='Trích xuất metadata từ đoạn văn. Trả về JSON thuần (không có markdown): {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}',
        user_content=text,
        max_tokens=150,
    )
...
def _enrich_single_call(text: str, source: str) -> dict:
    ...
    """
    result = _call_llm(
        system_prompt="""Phân tích đoạn văn và trả về JSON thuần (không markdown):
{
  ...
  "metadata": {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}
        ...
        user_content=f"Tài liệu: {source}\n\nĐoạn văn:\n{text}",
        max_tokens=400,
    )
